...
if not Config.ESX and not Config.Ace then
    print(
        '^1[bzn_allowlist] Error you need to set either Config.ESX og Config.Ace to true else you wont be able to use commands - script not loaded^7')
    return
end
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
    ...
    if not Identifiers or not Identifiers[Config.Identifier] then
        deferrals.done(_U('identifier_error', Config.Identifier))
        CancelEvent()
        return
    end
        ...
        if string.find(identifier, Config.Identifier .. ':') then
            ...
            elseif Config.Identifier == 'license' and string.len(identifier) == 40 then
                ...
                if Config.Identifier == 'steam' then
                    identifierLength = '15 or 17'
                elseif Config.Identifier == 'license' then
                    identifierLength = 40
                end

                SendMessage(source,
                    _U('identifier_length_error', Config.Identifier, identifierLength, Config.Identifier,
                        string.len(identifier)), 'error')
                return
            end
...
SaveToMySQL = function(identifier, priority)
    ...
    if priority ~= 0 and Config.EnableConnectQueue and Config.Identifier == 'steam' then
...
UpdateMySQL = function(identifier, priority)
    Allowlist[identifier] = priority
    ...
    if Config.EnableConnectQueue and Config.Identifier == 'steam' and priority ~= 0 then
        Queue.Exports:AddPriority(identifier, priority)
    end
end
