...
-- Returns the configured delay (ms) between light steps for a given vehicle.
-- Uses the resource convar 'els_lightDelay' fallback to `lightDelay` from config.lua.
-- Supports per-vehicle overrides via `vehicleLightDelayOverrides` in `config.lua`.
function getLightDelay(veh)
    local base = GetConvarInt("els_lightDelay", lightDelay or 50)
    local vkey = checkCarHash(veh)
    if vehicleLightDelayOverrides ~= nil and vehicleLightDelayOverrides[vkey] ~= nil then
        return vehicleLightDelayOverrides[vkey]
    end
