...
AddStateBagChangeHandler("VehicleProperties", nil, function(bagName, _, value)
    ...
    if not netId then
        print("^3[ESX Warning]^7 Tried to set vehicle properties with invalid netId")
        return
    end
    ...
    while not NetworkDoesEntityExistWithNetworkId(netId) do
        ...
        tries = tries + 1
        if tries > 20 then
            print(("^3[ESX Warning]^7 Invalid entity - ^5%s^7! (Entity may have been deleted or not synced yet)"):format(netId))
            return
        end
    end
    ...
    if not DoesEntityExist(vehicle) then
        print("^3[ESX Warning]^7 Vehicle entity does not exist after network sync")
        return
    end
