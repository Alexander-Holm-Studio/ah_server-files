...
if Config.Seatbelt.Enable then
    ESX.RegisterInput('esx_cruisecontrol:ToggleSeatbelt', Translate('toggleSeatbelt'), "keyboard", Config.Seatbelt.Key,
        function()
            if not Utils.vehicle then return end
            ...
            lib.notify({
                title = 'Seatbelt',
                description = Translate(SB.seatbelt and 'seatbeltOn' or 'seatbeltOff'),
                type = SB.seatbelt and 'success' or 'warning',
                icon = SB.seatbelt and 'user-check' or 'user-xmark',
                duration = 3000
            })
        end)
end
