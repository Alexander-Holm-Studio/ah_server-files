...
function OnPlayerDeath()
  ...
  if Config.DeathAnim.enabled then
    ...
    TaskPlayAnim(ESX.PlayerData.ped, Config.DeathAnim.dict, Config.DeathAnim.name, Config.DeathAnim.fadeIn,
      Config.DeathAnim.fadeOut,
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
  ...
  -- Example: print or use formattedPhoneNumber
  print('Your Danish phone number is: ' .. formattedPhoneNumber)
end)

-- Danish phone number formatter
local function FormatDanishPhoneNumber(number)
  number = tostring(number):gsub("%D", "")
  if #number == 8 then
    return number:sub(1, 2) .. " " .. number:sub(3, 4) .. " " .. number:sub(5, 6) .. " " .. number:sub(7, 8)
  else
    return number
  end
end
...
RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
  -- Format the phone number for Danish display
  local formattedPhoneNumber = FormatDanishPhoneNumber(phoneNumber)
end)
...
AddEventHandler('esx_ambulancejob:revive', function()
  ...
  local formattedCoords = { x = ESX.Math.Round(coords.x, 1), y = ESX.Math.Round(coords.y, 1), z = ESX.Math.Round(
  coords.z, 1) }
