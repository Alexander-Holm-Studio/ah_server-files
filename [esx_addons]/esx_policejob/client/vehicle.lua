					...
					for k, vehicle in ipairs(authorizedVehicles) do
						if IsModelInCdimage(vehicle.model) then
							...
							shopElements[#shopElements + 1] = {
								icon  = 'fas fa-car',
								title = ('%s - <span style="color:green;">%s</span>'):format(vehicleLabel,
						...
						ESX.OpenContext("right", garage, function(menuG, elementG)
							if elementG.stored == 1 or elementG.stored == true then
								...
								if foundSpawn then
									...
									local modelHash = tonumber(elementG.model) or joaat(elementG.model)

									ESX.Game.SpawnVehicle(modelHash, spawnPoint.coords, spawnPoint.heading,
										function(vehicle)
											local vehicleProps = allVehicleProps[elementG.plate]
											ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
											SetEntityAsMissionEntity(vehicle, true, true)

											TriggerServerEvent('esx_vehicleshop:setJobVehicleState', elementG.plate,
												false)
				...
				ESX.OpenContext("right", elements3, function(menu3, element3)
					...
					elseif element3.value == "buy" then
						...
						ESX.TriggerServerCallback('esx_policejob:buyJobVehicle', function(bought)
							if bought then
								ESX.ShowNotification(TranslateCap('vehicleshop_bought', element.name,
