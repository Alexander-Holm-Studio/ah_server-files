...
function TriggerCallback(event, cb, ...)
    ...
    TriggerServerEvent("lb-lookupapp:cb:" .. event, requestId, ...)
end
...
RegisterNetEvent("lb-lookupapp:cb:response", function(requestId, ...)
...
function SendReactMessage(event, data)
    exports["lb-phone"]:SendCustomAppMessage("lb-lookupapp", {
        action = event,
        data = data
    })
end
