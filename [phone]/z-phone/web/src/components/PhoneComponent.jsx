...
const PhoneComponent = ({ isShow }) => {
  ...
  const handleServiceMessageSubmit = async (e) => {
    e.preventDefault();
    if (!serviceMessage || !selectedService) return;

    const data = {
      job: selectedService.job,
      message: serviceMessage,
    };

    try {
      await axios.post("/send-message-service", data);
      setServiceMessage("");
    ...
    >
      ...
      >
        <div className="flex flex-col justify-center rounded-xl h-full w-full px-3">
          <div className="bg-slate-700 rounded-lg py-2 flex flex-col w-full p-3">
            <div className="flex justify-between items-center pb-2">
              <span className="truncate font-semibold">{selectedService?.service}</span>
              <div>
                <MdClose
                  className="text-2xl cursor-pointer text-white hover:text-red-500"
                  onClick={() => {
            ...
            <form onSubmit={handleServiceMessageSubmit} className="w-full">
              <div className="flex flex-col gap-1 py-2 text-xs">
                <textarea
                  value={serviceMessage}
                  onChange={(e) => setServiceMessage(e.target.value)}
                  placeholder="Describe your emergency or request..."
                  rows={4}
                  className="bg-slate-800 focus:outline-none text-white w-full text-xs resize-none no-scrollbar p-3 rounded-lg border border-slate-600"
                  required
                ></textarea>
                <div className="flex justify-end pt-2">
                  <button
      ...
      >
        ...
        <div
          className="pb-16"
          style={{
            ...(subMenu !== subMenuList["services"] ? { display: "none" } : {}),
          }}
        >
          {services?.list == undefined ? (
            ...
            <>
              {services.list.map((v, i) => {
                return (
                  <div
                    className="flex w-full justify-between border-b border-gray-900 pb-2 mb-2 items-center px-2 cursor-pointer hover:bg-gray-900"
                    key={i}
                  ...
                  >
                    ...
                    <div className="flex space-x-2 pr-3 items-center text-blue-500">
                        <MdOutlineCallMade className="text-2xl" />
                    </div>
                  </div>
                );
            ...
            <div
              ...
              onClick={async () => {
                try {
                  ...
                  // result = response.data; // Removed unused variable assignment
                } catch (error) {
      ...
      <div className="absolute bottom-0 w-full pb-2 pt-2.5 bg-black">
        <div className="grid h-full w-full grid-cols-3 mx-auto font-medium">
          ...
          <button
            type="button"
            className={`inline-flex flex-col items-center justify-center px-5 group ${
              subMenu === subMenuList["services"]
                ? "text-white"
                : "text-gray-600"
            }`}
            onClick={() => setSubMenu(subMenuList["services"])}
          >
