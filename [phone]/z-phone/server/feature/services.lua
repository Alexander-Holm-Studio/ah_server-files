...
lib.callback.register('z-phone:server:SendMessageService', function(source, body)
    ...
    -- Bridge to visualz_opkaldsliste - send the message to the call list tablet
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)

    -- Trigger the calllist system with the service message
    if GetResourceState('visualz_opkaldsliste') == 'started' then
        exports['visualz_opkaldsliste']:AddCall(source, body.message, body.job, playerCoords)
    end
