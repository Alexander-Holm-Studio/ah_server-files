...
AddEventHandler('wf-alerts:svNotify', function(pData)
	if not Blacklisted(source) then
		...
		elseif dispatchCodes[pData.dispatchCode] ~= nil then
			dispatchData =
				dispatchCodes[pData.dispatchCode]
		end
		...
		if pData.recipientList[1] == 'police' then
			TriggerEvent('mdt:newCall', details, pData.caller,
				vector3(pData.coords.x, pData.coords.y, pData.coords.z), false)
		end

		-- Integration with visualz_opkaldsliste
		if Config.OpkaldslisteIntegration and GetResourceState('visualz_opkaldsliste') == 'started' then
			for _, job in ipairs(pData.recipientList) do
				local callMessage = string.format("[%s] %s", pData.displayCode, pData.dispatchMessage)
				if pData.info then callMessage = callMessage .. " - " .. pData.info end
				if pData.info2 then callMessage = callMessage .. " | " .. pData.info2 end

				exports['visualz_opkaldsliste']:AddCall(source, callMessage, job, pData.coords)
			end
		end
	end
end)
...
AddEventHandler('wf-alerts:svNotify911', function(message, caller, coords)
	if message ~= nil then
		...
		-- Integration with visualz_opkaldsliste for 911 calls
		if Config.OpkaldslisteIntegration and GetResourceState('visualz_opkaldsliste') == 'started' then
			for _, job in ipairs(pData.recipientList) do
				local callMessage = string.format("[911] %s", message)
				exports['visualz_opkaldsliste']:AddCall(nil, callMessage, job, coords)
			end
		end
	end
end)
